...
CREATE OR REPLACE TASK SENSOR_LAB.SILVER.TASK_SILVER
    WAREHOUSE = COMPUTE_WH
...
CREATE OR REPLACE TASK SENSOR_LAB.GOLD.TASK_GOLD
    WAREHOUSE = COMPUTE_WH
    AFTER SENSOR_LAB.SILVER.TASK_SILVER
AS
    CALL SENSOR_LAB.GOLD.SP_AGGREGATE_TO_GOLD();
```
...
-- 子Taskから先に有効化すること！（重要）
ALTER TASK SENSOR_LAB.GOLD.TASK_GOLD RESUME;
ALTER TASK SENSOR_LAB.SILVER.TASK_SILVER RESUME;
